...
{
    private const string WordDocumentType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
    ...
    public async Task<IActionResult> OnPostDownloadWordAsync(int labNumber, string githubUserName, string fullName, string groupName, int pullNumber)
    {
        ...
        try
        {
            ...
            var inputModel = new InputModel(labNumber, fullName, groupName, githubProvider);

            byte[] reportByteArray = new WordCreator(inputModel, desc).CreateReport(byteArray).ToArray();

            return File(reportByteArray, WordDocumentType, inputModel.GetReportPath());
        }
